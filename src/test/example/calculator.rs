...
fn calculator() {
    ...
    let mononomial_complex = Seq::new("mononomial_complex", &mononomial_complex_slice);
    let mononomial_slice: Vec<&AsRuleKey> = vec![&two_term, &mononomial_complex];
    let mononomial = Choice::new("mononomial", &mononomial_slice);

    let polynomial_complex_slice: Vec<&AsRuleKey> = vec![&two_term, &plus_term, &"polynomial"];
    let polynomial_complex = Seq::new("polynomial_complex", &polynomial_complex_slice);
    let polynomial_slice: Vec<&AsRuleKey> = vec![&mononomial, &polynomial_complex];
    let polynomial = Choice::new("polynomial", &polynomial_slice);
    ...
    let item_set = build_items(&polynomial, &g, &alloc);
